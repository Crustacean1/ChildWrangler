...
pub fn Chart(padding: i32, series: Vec<(String, i32)>) -> impl IntoView {
    ...
    view! {
        ...
        .map(|(i,(start, end))| {
            ...
            view!{
                <path d={path} stroke=colors[i] stroke-width="12" fill="none" stroke-linecap="round" on:mousemove=move |e|{
                    e.dyn_into::<MouseEvent>().map(move |e| set_position(Some((i,e.layer_x(), e.layer_y()))));
                }
        ...
        <div class="pretty-background rounded padded anchor-end"
            style:position="absolute"
            style:visibility={move || position().map_or( String::from("hidden"), |_| String::new())}
            style:left={move || position().map(|(_,x,_)| format!("{}px", x + radius as i32 + 32 ))}
            style:top={move || position().map(|(_,_,y)| format!("{}px", y + radius as i32 - 16 ))}>
            {let series = series.clone();move || position().map(|(i,_,_)| format!("{}", series[i].0))}
        </div>
        <div class="grid-2 gap align-start justify-center">
            {series.iter().enumerate().map(|(i,(name,value))| view!{
...
pub fn AttendanceDashboard() -> impl IntoView {
    view! {
        ...
        {move || view!{
        <Chart padding={12} series=vec![(String::from("Entity 1"), 15), (String::from("Entity2"), 25),
